...
public class Signal3SoundInstance extends MovingSoundInstance {
    private final ClientPlayerEntity player;

    public Signal3SoundInstance(ClientPlayerEntity player) {
        super(WayfinderSounds.ALT_SIGNAL_3, SoundCategory.PLAYERS, SoundInstance.createRandom());
        this.player = player;
        this.repeat = true;
        this.repeatDelay = 0;
        this.volume = 0.01F;
    ...
    public void tick() {
        if (shouldPlay(this.player)){
            float raw = WayfinderComponents.WAYFINDER.get(player).getFactor3();
