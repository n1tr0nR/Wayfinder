...
public class Signal4SoundInstance extends MovingSoundInstance {
    private final ClientPlayerEntity player;

    public Signal4SoundInstance(ClientPlayerEntity player) {
        super(WayfinderSounds.DISTRESS, SoundCategory.PLAYERS, SoundInstance.createRandom()); //TODO: Add a beacon audio
        this.player = player;
        this.repeat = true;
        this.repeatDelay = 0;
        this.volume = 0.01F;
    ...
    public void tick() {
        if (shouldPlay(this.player)){
            float raw = WayfinderComponents.WAYFINDER.get(player).getFactor4();
            if (raw < 0.9F){
                this.volume = 0;
            } else {
